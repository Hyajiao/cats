...
public class ImageUtil {
        ...
        try {
            if (per >= 0 && per < 1f) {
                ...
                imageWriter.write(imageMetaData,
                        new IIOImage(newImg, null, null), jpegParams);
                ios.close();
                imageWriter.dispose();
                fos.close();
            } else {
    ...
    /**
     * 对于手机拍摄的图片，可能需要旋转。如果不需要旋转则返回原图片
     * @param targetFile
     * @return
     * @throws IOException
     */
    public static File doRotate(File targetFile) throws IOException {
        // 判断图片是否需要旋转（拍摄的原图都是jpeg格式）
        if(ImageUtil.isJpeg(new BufferedInputStream(new FileInputStream(targetFile)))){
            // 获取图片旋转角度
            int angel = getRotateAngle(targetFile);
            // 判断图片是否需要调整，翻转、调整图片
            if (angel != 0) {
                BufferedImage src = inputImage(targetFile.getAbsolutePath());
                BufferedImage bi = doRotate(src, angel);
    ...
    public static BufferedImage doRotate(Image src, int angel) {
        int srcWidth = src.getWidth(null);
        int srcHeight = src.getHeight(null);
        Rectangle rectDes = CalcRotatedSize(new Rectangle(
                new Dimension(srcWidth, srcHeight)), angel);

        BufferedImage bi = new BufferedImage(rectDes.width, rectDes.height,
                BufferedImage.TYPE_INT_RGB);
     ...
     * @throws IOException
     */
    public static boolean isJpeg(BufferedInputStream jpegInputStream) throws IOException {
    ...
    public static void main(String[] args) throws Exception {
        String srcImg = "D:/img/img_cover5.png";
        String tarDir = "D:/img/newImg/";

        long startTime = System.currentTimeMillis();

        ImageUtil.resizeFixedWidth(srcImg, tarDir + File.separator + "new-large_img_cover5.jpg", 180, 1F);

//        resize(srcImg, tarDir + "200.jpg", 200);
//        resize(srcImg, tarDir + "200-0.7.jpg", 200, 0.7F);
//
//        // Tosmallerpic(srcImg, tarDir + "car_1_maxLength_2.jpg", 0.5F);
//        resize(srcImg, tarDir + "400x500.jpg", 400, 500);
//        resize(srcImg, tarDir + "400x500-0.8.jpg", 400, 500, 0.8F);
//
//        resize(srcImg, tarDir + "220x220.jpg", 220, 220);



//        Thumbnails.of(srcImg).width(200).toFile(tarDir + "200-t.jpg");
//        Thumbnails.of(srcImg).height(200).toFile(tarDir + "200--ht.jpg");
//        Thumbnails.of(srcImg).width(200).outputQuality(0.7F).toFile(tarDir + "200-0.7-t.jpg");
//        Thumbnails.of(srcImg).size(400, 500).toFile(tarDir + "400x500-t.jpg");
//        Thumbnails.of(srcImg).size(400, 500).outputQuality(0.8F).toFile(tarDir + "400x500-0.8-t.jpg");
//        Thumbnails.of(srcImg).forceSize(220, 220).toFile(tarDir + "220x220-t.jpg");
