...
public final class CommonUtils {
	...
	private static final String[] WELCOME_FILES = {"index.html","index.htm","index.do","index.jsp"};
	...
	public static void readAndRespondToProxyReceptorRequest(final HttpServletRequest request,
		...
		if (LOG.isDebugEnabled()) {
			LOG.debug("Received proxyGrantingTicketId [" + proxyGrantingTicket + "] for proxyGrantingTicketIou ["
		...
		if (LOG.isDebugEnabled()) {
			LOG.debug("Successfully saved proxyGrantingTicketId [" + proxyGrantingTicket
	...
	public static String constructServiceUrl(final HttpServletRequest request, final HttpServletResponse response,
		...
		if (CommonUtils.isNotBlank(request.getQueryString())) {
			...
			if (location == 0) {
				// UPDATE 回复默认的处理方法，不对welcome URL做特殊处理  BY peihong 20121221 START
				final String returnValue = encode ? response.encodeURL(buffer.toString()) : buffer.toString();
//				String url =getReturnUrl(buffer);
//				final String returnValue = encode ? response.encodeURL(url) : url;
				// UPDATE 回复默认的处理方法，不对welcome URL做特殊处理  BY peihong 20121221 END
				if (LOG.isDebugEnabled()) {
...
/** 2012-08-23 liuld upd begin 因为welcome file url 的验证问题*/
		// UPDATE 回复默认的处理方法，不对welcome URL做特殊处理  BY peihong 20121221 START
		final String returnValue = encode ? response.encodeURL(buffer.toString()) : buffer.toString();
...
/**2012-08-23 liuld upd end  */
	...
	private static String getReturnUrl(StringBuilder buffer) {
		String url = buffer.toString();
		String temp = url.substring(url.lastIndexOf("/") + 1);
		for(String welcome_file : WELCOME_FILES) {
			if(welcome_file.equalsIgnoreCase(temp)) {
